...
const double pB_offset   =  1.0;
const double pS_offset   =  0.0;
const double pK18_offset =  0.0;
const double pSKS_offset =  0.0;
const double bh2_dE      =  0.0;
...
EventPiPiAnaTheta::EventPiPiAnaTheta()
  : VEvent(),
    rawData(0), 
...
struct Event{
  ...
  int matrixpat[NumOfMatrix];
  int matrixflag[NumOfMatrix];
...
// TOF Cut Parameters
const double MinDeltaETof = 0.0; //for Kaon
const double MaxDeltaETof = 3.0; //for Kaon
const double MinTimeTof =  15.0;
const double MaxTimeTof =  30.0;

// LC Cut Parameters
const double MinDeltaELc = 0.0; //for Kaon
const double MaxDeltaELc = 3.0; //for Kaon
const double MinTimeLc =  20.0;
const double MaxTimeLc =  35.0;

// TOF-LC  
const double MinToFT2L =    0.0;
const double MaxToFT2L =   10.0;
const int MinToF2LSeg  =    0.0;
const int MaxToF2LSeg  =   10.0;

//SdcIn vs TOF&LC cut
const double MinModTimeTof = 18.0;
const double MaxModTimeTof = 22.0;
...
const double MinMassSquare = -0.05;
const double MaxMassSquare =  0.15;
...
bool EventPiPiAnaTheta::ProcessingBegin()
...
bool EventPiPiAnaTheta::ProcessingNormal()
{
  rawData = new RawData;
  rawData->DecodeHits();
  ...
  int matrix_flag=0;
  {
    const HodoRHitContainer &cont=rawData->GetMatrixRawHC();
    int nh=cont.size();
    for( int i=0; i<nh; ++i ){
      HodoRawHit *hit=cont[i];
      int seg=hit->SegmentId()+1;
      ...
      if( T>0 ) event.matrixpat[i] = seg;
      event.matrixflag[seg-1] = T;
      if( seg==1 && T>0 ){ matrix_flag = seg; }
    }
  }
  ...
  //if ( matrix_flag==0 ) return true;

//   std::cout << "PiK==" << trig_type <<std::endl;
//   std::cout << "Flag==" << matrix_flag <<std::endl;
  ...
  HF1( 301, double(ncTof) );
  {
	...
	event.deTof[i]=de;
	  ...
	  MinTimeTof  <t  && t<MaxTimeTof && (cl->MeanSeg()+1)>8 ){
	++ncOk;
  ...
  HF1( 401, double(ncLc) );
  {
	...
	event.deLc[i]=de;
	  ...
	  MinTimeLc  <t  && t<MaxTimeLc && (cl->MeanSeg()+1)>7 ){
	++ncOk;
  ...
  HF1( 1, 12. );
  {
	...
	//------------------------Cut
	if( MinModTimeTof< ttof+10.0*u0 && ttof+10.0*u0 <MaxModTimeTof )
	  condTof=true;
      }
...
//             MinYDifLc<ylc-y && ylc-y<MaxYDifLc )
  ...
  for( int ik=0; ik<nK; ++ik ){
    ...
    for( int ipi=0; ipi<nPi; ++ipi ){
      ...
      ThreeVector pkCorrDE  = CorrElossOut( pkCorr, xk, vert, MuonMass);
      ...
      LorentzVector LvK( pk, sqrt(MuonMass*MuonMass+pk.mag2()) );
      LorentzVector LvKCorr( pkCorr, sqrt(MuonMass*MuonMass+pkCorr.mag2()) );
      LorentzVector LvKCorrDE( pkCorrDE, sqrt(MuonMass*MuonMass+pkCorrDE.mag2()) );
...
//       LorentzVector LvKCorrDE( pkCorrDE, sqrt(KaonMass*KaonMass+pkCorrDE.mag2()) );
	...
	= 0.5*sqrt(( TotalEnergyCM*TotalEnergyCM
		     -( MuonMass+ThetaMass )*( MuonMass+ThetaMass ))
		   *( TotalEnergyCM*TotalEnergyCM
		      -( MuonMass-ThetaMass )*( MuonMass-ThetaMass )))/TotalEnergyCM;
      
      double costLab = cost;
      double cottLab = costLab/sqrt(1.-costLab*costLab);
      double bt=beta.mag(), gamma=1./sqrt(1.-bt*bt);
      double gbep=gamma*bt*sqrt(TotalMomCM*TotalMomCM+MuonMass*MuonMass)/TotalMomCM;
...
//       double TotalMomCM 
// 	= 0.5*sqrt(( TotalEnergyCM*TotalEnergyCM
// 		     -( KaonMass+ThetaMass )*( KaonMass+ThetaMass ))
// 		   *( TotalEnergyCM*TotalEnergyCM
// 		      -( KaonMass-ThetaMass )*( KaonMass-ThetaMass )))/TotalEnergyCM;
...
void EventPiPiAnaTheta::InitializeEvent( void )
{
  ...
  for( int it=0; it<NumOfMatrix; it++){
    event.matrixpat[it] = -1;
    event.matrixflag[it] = -1;
  }
...
bool EventPiPiAnaTheta::ProcessingEnd()
{
  // gFile->Write();
  // gFile->Close();
  return true;
...
{  
  ...
  //Misc
  tree->Branch("matrixpat",  event.matrixpat,   "matrixpat[10]/I");
  tree->Branch("matrixflag", event.matrixflag,  "matrixflag[10]/I");
