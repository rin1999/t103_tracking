...
const int DCLocalMinNHits = 6;
const int DCLocalMinNHitsVXU = 3;
...
/*New Added by K.Miwa for geometory check*/
bool DCLocalTrack::DoFitBcSdc( void )
{
  const std::string funcname = "[DCLocalTrack::DoFitBcSdc()]";
  const DCGeomMan & geomMan=DCGeomMan::GetInstance();
  ...
  double zK18tgt=geomMan.GetLocalZ( 130 );
  for( std::size_t i=0; i<n; ++i ){
    DCLTrackHit *hitp = hitArray[i];
    if( hitp ){
      int lnum = hitp->GetLayer();
...
#if 0
      std::cout << std::setw(2) << i << " (" << lnum << ") " 
                << std::setw(3) << hitp->GetWire() << " "
                << hitp->GetLocalHitPos() << " "
                << zz << " " << aa 
                << std::endl;
#endif
...
/////// 2010/6/8 Y.Y
#if 1
bool DCLocalTrack::DoFitVXU( void )
{
  const std::string funcname = "[DCLocalTrack::DoFitVXU()]";
  const DCGeomMan & geomMan=DCGeomMan::GetInstance();

  std::size_t n = hitArray.size();

  if(n < DCLocalMinNHitsVXU ) return status_ = false;
...
// std::size_t DCLocalTrack::DSSize( void ) const
// {
//   static const std::string funcname = "[DCLocalTrack::DSSize]";

//   std::size_t size=sizeof(DSFormat)/sizeof(int);
//   if( sizeof(DSFormat)%sizeof(int) ) size+=1;
  
//   int nh=hitArray.size();
//   for( int i=0; i<nh; ++i )
//     if( DCLTrackHit *hit=hitArray[i] )
//       size += hit->DSSize();
...
// std::size_t DCLocalTrack::DSSave( unsigned int *bufp ) const
// {
//   static const std::string funcname = "[DCLocalTrack::DSSave]";

//   DSFormat data;
//   std::size_t size=sizeof(data)/sizeof(int);
//   if( sizeof(data)%sizeof(int) ) size+=1;
//   std::size_t totalSize=DSSize();

//   int nh=hitArray.size();
//   int headerId=DSMakeHeaderID( SksObjDCLocalTrack );
//   data.header_=DSMakeHeader( headerId, totalSize );
//   data.x0_=x0_; data.y0_=y0_; data.u0_=u0_; data.v0_=v0_;
//   data.chisqr_=chisqr_; data.numofhits_=nh;
//   if( status_ ) data.status_=1;
//   else          data.status_=0;
//   if( gftstatus_ ) data.gftstatus_=1;
//   else             data.gftstatus_=0;
...
//   for( int i=0; i<nh; ++i ){
//     if( DCLTrackHit *hit=hitArray[i] ){
//       size=hit->DSSave(bufp); bufp+=size;
//       int headerId2=headerId;
//       int lnum=hit->GetLayer();
//       if( lnum>=PlMinBcIn+PlOffsBc && lnum<=PlMaxBcIn+PlOffsBc )
// 	headerId2=DSMakeHeaderID( SksObjLocalTrackBIC );
//       else if( lnum>=PlMinBcOut+PlOffsBc && lnum<=PlMaxBcOut+PlOffsBc )
// 	headerId2=DSMakeHeaderID( SksObjLocalTrackBOC );
//       else if( lnum>=PlMinSdcIn && lnum<=PlMaxSdcIn )
// 	headerId2=DSMakeHeaderID( SksObjLocalTrackSIC );
//       else if( lnum>=PlMinSdcOut && lnum<=PlMaxSdcOut )
// 	headerId2=DSMakeHeaderID( SksObjLocalTrackSOC );
      
//       hit->DSSetSaveKey( DSMakeKey( headerId2, i ) );
//     }
//   }
...
// bool DCLocalTrack::DSRestore( unsigned int *bufp, DCAnalyzer *DCana )
// {
//   static const std::string funcname = "[DCLocalTrack::DSRestore]";
...
//   x0_=data.x0_; y0_=data.y0_; u0_=data.u0_; v0_=data.v0_;
//   chisqr_=data.chisqr_; 
//   if( data.status_ ) status_=true;
//   else               status_=false;

//   if( data.gftstatus_ ) gftstatus_=true;
//   else                  gftstatus_=false;

//   int nh=data.numofhits_;
//   hitArray.clear(); hitArray.reserve(nh);

//   while( bufp < hp+totalSize ){
//     size = DSGetSize( *bufp );
//     int head = DSGetHeaderID( *bufp );
//     int objId = DSGetObjectID( head );
// #if 0    
//     std::cout << funcname << ": Size=" << std::setw(4) << size
//               << " objId=" << std::setw(3) << objId << std::endl;
// #endif
//     if( objId == SksObjDCLTrackHit ){
//       DCLTrackHit *hit=new DCLTrackHit( bufp, DCana );
//       if( hit ) hitArray.push_back(hit);
//       else {
