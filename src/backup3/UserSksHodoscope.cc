/*
  ...
  2005/02/21  K6 Original
  2010/02     modified for K1.8 by M.Moritsu
*/
...
const int PIK_TRIG   = 5;
const int PIPI_TRIG  = 6;
const int PIP_TRIG   = 7;
...
//const double BH2toTgt = 7.0;//Time difference between BH1 and Target [ns]  

// TOF Cut Parameters
const double MinDeltaETof = 0.4; //for Kaon
const double MaxDeltaETof = 1.6; //for Kaon
const double MinTimeTof =  20.0;
const double MaxTimeTof =  30.0;

//const double MinTimeTof =  5.5;
//const double MaxTimeTof = 35.0;
//const double MinDeltaETof = 0.6;   /* for Pion */

// LC Cut Parameters
const double MinDeltaELc = 0.01; //for Kaon
const double MaxDeltaELc = 3.5; //for Kaon
const double MinTimeLc =  23.0;
const double MaxTimeLc =  38.0;

//const double MinTimeLc = 12.5;
//const double MaxTimeLc = 40.0;
//const double MinDeltaELc  = 0.1;   /* for Pion */

// TOF-LC  
const double MinToFT2L =   1.0;
const double MaxToFT2L =  13.0;
const int MinToF2LSeg  =   0;
const int MaxToF2LSeg  =  12;
// const int MinToF2LSeg  =  0;
// const int MaxToF2LSeg  =  12;
 
//const int MinSegDifTL = -10;
//const int MaxSegDifTL =  10;
//const double MinToFT2L = -2.5;
//const double MaxToFT2L = 12.5;
//const double MinToFT2L = -1.0;
//const double MaxToFT2L = 15.0;

// TrackChiSqr
const double MaxChisqrSks    = 100.;
const double MaxChisqrBdcIn  = 100.;
const double MaxChisqrBdcOut = 100.;
const double MaxChisqrSdcIn  = 100.;
const double MaxChisqrSdcOut = 100.;

// TOF-SdcOut
const double MinXDifTof  =  -100.;
const double MaxXDifTof  =   100.;
const double MinYDifTof  =  -200.;
const double MaxYDifTof  =   200.;

// LC-SdcOut
const double MinXDifLc  = -200.;
const double MaxXDifLc  =  200.;
const double MinYDifLc  = -600.;
const double MaxYDifLc  =  600.;
...
class EventSksHodoscope : public VEvent
{
...
EventSksHodoscope::EventSksHodoscope()
  : VEvent(), 
    rawData(0), 
...
bool EventSksHodoscope::ProcessingBegin()
...
bool EventSksHodoscope::ProcessingNormal()
{
  static const std::string funcname = "[EventSksHodoscope::ProcessingNormal]";
 ...
 //Misc
  ...
  int trig=0;
  {
    ...
    for( int i=0; i<nh; ++i ){
      ...
      //(pi, K)
      if( seg==5 && T>0 ){ trig_type = seg; event.trigtype = seg;}
      //(pi, pi)
      //if( seg==6 && T>0 ){ trig_type = seg; event.trigtype = seg;}
      //(pi, p)
      //if( seg==7 && T>0 ){ trig_type = seg; event.trigtype = seg;}
  ...
  //if (trig_type != PIP_TRIG)  return true;
  ...
  HF1( BH2Hid+10, double(nhBh2) );
  if( nhBh2==0 ){
  //if( nhBh2!=1 ){
    //std::cout<<"#D NHitsBH2 != 1 [return]"<<std::endl; 
    return true;
  }
  double time0=hodoAna->GetHitBH2(0)->CTime0();
  HF1( BH2Hid+50, time0 );
  ...
  event.nhLc=ncLc;
  {
	...
	x=(ms-14.5)*100., y=dt*50.;
      ...
      //------------------------Cut
      if( de>MinDeltaELc && de<MaxDeltaELc &&
	  t>MinTimeLc && t<MaxTimeLc ){
        ++ncOk;
  ...
  // pre Event Selection by TOF-LC
  {
	...
	if( segTof-segLc>=MinToF2LSeg && segTof-segLc<=MaxToF2LSeg
	    && tlc-ttof>MinToFT2L && tlc-ttof<MaxToFT2L ){
          ++nc;
  ...
  event.ntOut=ntOut;
  {
	...
	if( x0-x>MinXDifTof && x0-x<MaxXDifTof &&
            y0-y>MinYDifTof && y0-y<MaxYDifTof )
          condTof=true;
      }
	...
	if( xlc-x>MinXDifLc && xlc-x<MaxXDifLc &&
            ylc-y>MinYDifLc && ylc-y<MaxYDifLc )
          condLc=true;
      }
      if( condTof && condLc ){
...
void EventSksHodoscope::InitializeEvent( void )
{
...
bool EventSksHodoscope::ProcessingEnd()
{
  // gFile->Write();
  // gFile->Close();
