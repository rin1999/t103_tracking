...
// bool MathTools::Interpolate( int n, 
...
//   double *w1=y1p, *w2=y, *w3=y3p;
//   w1[0]=0.; w3[0]=0.;
//   for(int i=1; i<=n-2; ++i ){
//     w1[i]=0.5*d[i-1]/(d[i-1]+d[i]);
//     w3[i]=3.*((a[i+1]-a[i])/d[i]-(a[i]-a[i-1])/d[i-1])/(d[i-1]+d[i]);
//   }
//   w1[n-1]=-1.; w3[n-1]=0.;
...
//   for(int i=2; i<n-1; ++i ){
//     w2[i]=(0.5-w1[i-1]);
//     y4p[i]=(w3[i]-w1[i]*w3[i-1])/(1.-w1[i]*w2[i]);
//   }
//   w2[n-1]=0.;
//   y4p[n-1]=0.;
...
//   for(int i=0; i<n-1; ++i ){
//     double d2=d[i]*d[i];
//     y1p[i+1] = y1p[i]+0.5*d[i]*(a[i+1]+a[i])-d2*d[i]/24.*(y4p[i]+y4p[i+1]);
//     y[i+1]   = y[i]+y1p[i]*d[i]+d2/6.*(2.*a[i]+a[i+1])
//               -d2*d2/36.*(0.8*y4p[i]+0.7*y4p[i+1]);
//     y3p[i]   = (a[i+1]-a[i])/d[i]-d[i]/6.*(2.*y4p[i]+y4p[i+1]);
//   }
//   y3p[n-1]=y3p[n-2]+d[n-2]*y4p[n-2];
...
// bool MathTools::SolveRay( int n, const std::vector<double> x, 
// 			  const std::vector<double> y, 
// 			  const double *yi2,  
// 			  const std::vector<double> w, double *p )
// {
//   double matrx[9], *mtp[3];
//   int indx[3], indy[3], ipiv[3];

//   mtp[0]=&matrx[0]; mtp[1]=&matrx[3]; mtp[2]=&matrx[6];
//   for(int i=0; i<3; ++i ){
//     p[i]=0.0;
//     for(int j=0; j<3; ++j ) mtp[i][j]=0.0;
//   }
...
//     mtp[0][2] += w[i]*yi2[i];
//     mtp[1][1] += w[i]*x[i]*x[i];
//     mtp[1][2] += w[i]*x[i]*yi2[i];
//     mtp[2][2] += w[i]*yi2[i]*yi2[i];
//     p[0]      += w[i]*y[i];
//     p[1]      += w[i]*y[i]*x[i];
//     p[2]      += w[i]*y[i]*yi2[i];
//   }
//   mtp[1][0]=mtp[0][1]; mtp[2][0]=mtp[0][2]; mtp[2][1]=mtp[1][2];
...
//   //  return MathTools::GaussElim(mtp,3,p,indx,ipiv);
//   return MathTools::GaussJordan(mtp,3,p,indx,indy,ipiv);
// }
