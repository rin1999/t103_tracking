...
Scat2BTrack::Scat2BTrack( double IniP, 
			PreInTrack *In, 
			TrLocalTrack *Out )
  : In_(In), Out_(Out),
    iniP_(IniP+DefPini),
...
Scat2BTrack::~Scat2BTrack()
{
  clearHitArray();
}

TrackHit *Scat2BTrack::GetHit( std::size_t nth ) const
{
...
TrackHit *Scat2BTrack::GetHitOfLayerNumber( int lnum ) const
{
...
void Scat2BTrack::fillHitArray( void )
{
...
void Scat2BTrack::clearHitArray( void )
{
...
bool Scat2BTrack::ReCalc( bool applyRecursively )
{
  static const std::string funcname = "[Scat2BTrack::ReCalc]";
...
bool Scat2BTrack::doFit( void )
{
  static const std::string funcname = "[Scat2BTrack::doFit]";
  ...
  RKScat2BCordParameter iniCord( gpTof, gmTof );
  RKScat2BCordParameter prevCord;

  RKScat2BHitPointContainer preHPntCont;
  ...
  hPntCont_ = RKScat2BmakeHPContainer(); 
  RKScat2BHitPointContainer prevHPntCont;

  int iIter=0, iIterEf=1;

  while( ++iIter < MaxIteraction ){
    if( !RKScat2Btrace( iniCord, hPntCont_ ) ){
      // Error 
#ifdef WARNOUT 
      //std::cerr << funcname << ": Error in RKScat2Btrace. " << std::endl;
#endif
      break;
    }
  ...
  if( !RKScat2BtraceToLast( hPntCont_ ) )
    status_ = false;
...
bool Scat2BTrack::doFitAgain( RKScat2BCordParameter iniCord )
{
  static const std::string funcname = "[Scat2BTrack::doFitAgain]";
...
double Scat2BTrack::calcChiSqr( const RKScat2BHitPointContainer &hpCont ) const
{
  ...
  for( std::size_t i=0; i<nh; ++i ){
    ...
    const RKScat2BcalcHitPoint &calhp = hpCont.HitPointOfLayer( lnum );
...
bool Scat2BTrack::
guessNextParameters( const RKScat2BHitPointContainer &hpCont,
		     RKScat2BCordParameter &Cord, double &estDeltaChisqr, 
		     double &lambdaCri,  double dmp ) const
{
  static const std::string funcname = "[Scat2BTrack::geussNextParameters]";
  ...
  Cord = RKScat2BCordParameter( Cord.X(),
			       Cord.Y()+dcb[0],
			       Cord.Z()+dcb[1],
			       Cord.U()+dcb[2],
			       Cord.V()+dcb[3],
...
void Scat2BTrack::saveCalcPosition( const RKScat2BHitPointContainer &hpCont )
{
  static const std::string funcname = "[Scat2BTrack::saveCalcPosition]";
...
void Scat2BTrack::PrintCalcHits( const RKScat2BHitPointContainer &hpCont,
				std::ostream &ost ) const
{
  std::size_t ndh = hitArray_.size();

  RKScat2BHitPointContainer::RKScat2BHpCIterator 
  ...
  for( ; itr!=end; ++itr ){
    int lnum = itr->first;
    const RKScat2BcalcHitPoint &calhp = itr->second;
...
bool Scat2BTrack::saveTrackParameters( const RKScat2BCordParameter &cp )
{
  CPval_ = cp;
  const TrGeomMan &geomMan = TrGeomMan::GetInstance();
  int TOFid = geomMan.GetDetectorId("ITOFR");
  int TGTid = hPntCont_.begin()->first;

  const RKScat2BcalcHitPoint &hpTof  = hPntCont_.HitPointOfLayer( TOFid );
  const RKScat2BcalcHitPoint &hpTgt  = hPntCont_.begin()->second;
  const RKScat2BcalcHitPoint &hpLast = hPntCont_.rbegin()->second;
...
GetTrajectoryLocalPosition( int layer, double & x, double & y ) const
{
  static const std::string funcname = 
    "[Scat2BTrack:GetTrajectoryLocalPosition]";

  try {
    const RKScat2BcalcHitPoint &HP=hPntCont_.HitPointOfLayer( layer );
