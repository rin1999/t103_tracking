...
Scat3Track::Scat3Track( double IniP, 
			TrLocalTrack *In1, 
			TrLocalTrack *In2 )
  : In1_(In1), In2_(In2),
    iniP_(IniP+DefPini), 
...
Scat3Track::~Scat3Track()
{
  clearHitArray();
}

TrackHit *Scat3Track::GetHit( std::size_t nth ) const
{
...
TrackHit *Scat3Track::GetHitOfLayerNumber( int lnum ) const
{
...
void Scat3Track::fillHitArray( void )
{
  std::size_t nIn1 = In1_->GetNHit();
  std::size_t nIn2 = In2_->GetNHit();
 
  clearHitArray();
  hitArray_.reserve( nIn1+nIn2 );

  for( std::size_t i=0; i<nIn1; ++i ){
    TrLTrackHit *hit = In1_->GetHit( i );
    TrackHit *thit = new TrackHit( hit );
    hitArray_.push_back( thit );
  }

  for( std::size_t i=0; i<nIn2; ++i ){
    TrLTrackHit *hit = In2_->GetHit( i );
    TrackHit *thit = new TrackHit( hit );
    hitArray_.push_back( thit );
  }

  //std::cout<< nIn1 << " " << nIn2 <<std::endl;  
}

void Scat3Track::clearHitArray( void )
{
...
bool Scat3Track::ReCalc( bool applyRecursively )
{
  static const std::string funcname = "[Scat3Track::ReCalc]";

  if( applyRecursively ){
    In1_->ReCalc(applyRecursively);
    In2_->ReCalc(applyRecursively);
...
bool Scat3Track::doFit( void )
{
  static const std::string funcname = "[Scat3Track::doFit]";
  ...
  double xTof = In2_->GetX( zTof );
  double yTof = In2_->GetY( zTof );
  double uTof = In2_->GetU0(); 
  double vTof = In2_->GetV0(); 
 ...
 ThreeVector gpTof = TrGeomMan::GetInstance().
  ...
  RKScat3CordParameter iniCord( gpTof, gmTof );
  RKScat3CordParameter prevCord;

  RKScat3HitPointContainer preHPntCont;
  ...
  hPntCont_ = RKScat3makeHPContainer(); 
  RKScat3HitPointContainer prevHPntCont;

  int iIter=0, iIterEf=1;

  while( ++iIter < MaxIteraction ){
    if( !RKScat3trace( iniCord, hPntCont_ ) ){
      // Error 
#ifdef WARNOUT 
      //std::cerr << funcname << ": Error in RKScat3trace. " << std::endl;
#endif
      break;
    }
  ...
  if( !RKScat3traceToLast( hPntCont_ ) )
    status_ = false;
...
bool Scat3Track::doFitAgain( RKScat3CordParameter iniCord )
{
  static const std::string funcname = "[Scat3Track::doFitAgain]";
...
double Scat3Track::calcChiSqr( const RKScat3HitPointContainer &hpCont ) const
{
  ...
  for( std::size_t i=0; i<nh; ++i ){
    ...
    const RKScat3calcHitPoint &calhp = hpCont.HitPointOfLayer( lnum );
...
bool Scat3Track::
guessNextParameters( const RKScat3HitPointContainer &hpCont,
		     RKScat3CordParameter &Cord, double &estDeltaChisqr, 
		     double &lambdaCri,  double dmp ) const
{
  static const std::string funcname = "[Scat3Track::geussNextParameters]";
  ...
  Cord = RKScat3CordParameter( Cord.X(),
			       Cord.Y()+dcb[0],
			       Cord.Z()+dcb[1],
			       Cord.U()+dcb[2],
			       Cord.V()+dcb[3],
...
void Scat3Track::saveCalcPosition( const RKScat3HitPointContainer &hpCont )
{
  static const std::string funcname = "[Scat3Track::saveCalcPosition]";
...
void Scat3Track::PrintCalcHits( const RKScat3HitPointContainer &hpCont,
				std::ostream &ost ) const
{
  std::size_t ndh = hitArray_.size();

  RKScat3HitPointContainer::RKScat3HpCIterator 
  ...
  for( ; itr!=end; ++itr ){
    int lnum = itr->first;
    const RKScat3calcHitPoint &calhp = itr->second;
...
bool Scat3Track::saveTrackParameters( const RKScat3CordParameter &cp )
{
  CPval_ = cp;
  const TrGeomMan &geomMan = TrGeomMan::GetInstance();
  int TOFid = geomMan.GetDetectorId("SpecVp2");
  int TGTid = hPntCont_.begin()->first;

  const RKScat3calcHitPoint &hpTof  = hPntCont_.HitPointOfLayer( TOFid );
  const RKScat3calcHitPoint &hpTgt  = hPntCont_.begin()->second;
  const RKScat3calcHitPoint &hpLast = hPntCont_.rbegin()->second;
...
GetTrajectoryLocalPosition( int layer, double & x, double & y ) const
{
  static const std::string funcname = 
    "[Scat3Track:GetTrajectoryLocalPosition]";

  try {
    const RKScat3calcHitPoint &HP=hPntCont_.HitPointOfLayer( layer );
