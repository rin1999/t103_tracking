...
const int ReservedNumOfHits = 12;
const int TrLocalMinNHits = 5;

TrLocalTrack::TrLocalTrack()
  : status_(false), x0_(0.0), y0_(0.0), u0_(0.0), v0_(0.0),
    gftstatus_(true), mfiber_l1_(-1.0), mfiber_l2_(-1.0), mfiber_l3_(-1.0), 
    mfiber_l4_(-1.0), mfiber_l5_(-1.0), mfiber_l6_(-1.0)
{
...
//std::vector<int> *TrLocalTrack::GetHitLayers() const
//{
//  bool flag_layers[6] = {false,false,false,false,false,false};
//  std::vector<int>* hitlayers;
//  for(int i=0; i<hitArray.size(); i++){
//    int layer = hitArray[i]->GetLayer();
//    flag_layers[layer-1] = true;
//  }
//  for(int i=0; i<6; i++){
//    if(flag_layers[i]==true) hitlayers->push_back(i+1);
//  }
//  return hitlayers;
//}
...
bool TrLocalTrack::DoFit( void )
{
  ...
  for( std::size_t i=0; i<n; ++i ){
    ...
    if( hitp ){
      ...
      {
      case 1:
        mfiber_l1_ = hitp->GetMeanFiber();
        break;
      case 2:
        mfiber_l2_ = hitp->GetMeanFiber();
        break;
      case 3:
        mfiber_l3_ = hitp->GetMeanFiber();
        break;
      case 4:
        mfiber_l4_ = hitp->GetMeanFiber();
        break;
      case 5:
        mfiber_l5_ = hitp->GetMeanFiber();
        break;
      case 6:
        mfiber_l6_ = hitp->GetMeanFiber();
        break;
      ...
      std::cout << std::setw(10) << "layer = " << lnum 
		<< std::setw(10) << "fiber  = " << hitp->GetFiber() << " "
		<< std::setw(20) << "Position = "<<hitp->GetPosition() << " "
