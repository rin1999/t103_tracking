...
bool RawData::AddDCRHit( DCRHitContainer& cont, int LayerId, int WireId, DoubleVec lTdc, DoubleVec Tot )
{
...
bool RawData::AddTrRHit( TrRHitContainer& cont, int LayerId, int FiberId, DoubleVec lTdc, DoubleVec Tot )
{
   static const std::string funcname = "[RawData::AddDCRHit]";
...
bool RawData::AddHodoRHit( HodoRHitContainer& cont, int DetId, int LayId, int SegId, DoubleVec lTdc1, DoubleVec Tot1, DoubleVec lTdc2, DoubleVec Tot2)
{
   ...
   if(p){ // ここでhodoscopeのltdcをセットしている
      p->SetlTdc1( lTdc1 );
      p->SetlTdc2( lTdc2 );
      p->SetTot1( Tot1 );
      p->SetTot2( Tot2 );
...
bool RawData::DecodeRawHits( TFile* iFile, int i_entry )
{
  ...
  //ここでUTOFの平均LTDCを計算（未完）
  const double min_ltdc_utof_r = 990;
  const double max_ltdc_utof_r = 1010;
  const double min_tot_utof_r  = 1.0;
  bool flag_utof_r = false;
  double ltdc_utof_mean = -99999;
      for(int i=0; i<ltdc_utof_r->at(0).size(); i++){
         if(min_ltdc_utof_r<ltdc_utof_r->at(0).at(i) && ltdc_utof_r->at(0).at(i)<max_ltdc_utof_r && min_tot_utof_r<tot_utof_l->at(0).at(0) && min_tot_utof_r<tot_utof_r->at(0).at(i) && flag_utof_r==false){
            flag_utof_r = true;
  ...
  for(int i=0; i<NumOfSegUTOF; i++){
     AddHodoRHit(UTOFRHC, DetIdUTOF, 0, i+1, ltdc_utof_l->at(i), tot_utof_l->at(i), ltdc_utof_r->at(i), tot_utof_r->at(i) );
  }

  for(int i=0; i<NumOfSegDTOF; i++){
     AddHodoRHit(DTOFRHC, DetIdDTOF, 0, i+1, ltdc_dtof_l->at(i), tot_dtof_l->at(i), ltdc_dtof_r->at(i), tot_dtof_r->at(i) );
  }

  for(int i=0; i<NumOfSegLTOF; i++){
     AddHodoRHit(LTOFRHC, DetIdLTOF, 0, i+1, ltdc_ltof_l->at(i), tot_ltof_l->at(i), ltdc_ltof_r->at(i), tot_ltof_r->at(i) );
  }

  for(int i=0; i<NumOfSegT0; i++){
     AddHodoRHit(T0RHC, DetIdT0, 0, i+1, ltdc_t0_l->at(i), tot_t0_l->at(i), ltdc_t0_r->at(i), tot_t0_r->at(i) );
  }

  for(int i=0; i<NumOfSegT0r; i++){
     AddHodoRHit(T0rRHC, DetIdT0r, 0, i+1, ltdc_t0r_l->at(i), tot_t0r_l->at(i), ltdc_t0r_r->at(i), tot_t0r_r->at(i) );
  }

  for(int i=0; i<NumOfSegBref; i++){
     if(i==0){
        AddHodoRHit(BrefRHC, DetIdBref, 0, i+1, ltdc_bref_l->at(i), tot_bref_l->at(i), ltdc_bref_l->at(i), tot_bref_l->at(i) );
     }
     if(i==1){
        AddHodoRHit(BrefRHC, DetIdBref, 0, i+1, ltdc_bref_r->at(i-1), tot_bref_r->at(i-1), ltdc_bref_r->at(i-1), tot_bref_r->at(i-1) );
     }
  }
  
  for(int i=0; i<NumOfSegT1; i++){
     AddHodoRHit(T1RHC, DetIdT1, 0, i+1, ltdc_t1_l->at(i), tot_t1_l->at(i), ltdc_t1_r->at(i), tot_t1_r->at(i) );
  }

  for(int i=0; i<NumOfSegBHT; i++){
     AddHodoRHit(BHTRHC, DetIdBHT, 0, i+1, ltdc_bht_l->at(i), tot_bht_l->at(i), ltdc_bht_r->at(i), tot_bht_r->at(i) );
  }
