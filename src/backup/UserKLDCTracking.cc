...
class EventKLDCTracking : public VEvent
{
public:
   EventKLDCTracking();
   ~EventKLDCTracking();
...
EventKLDCTracking::EventKLDCTracking()
   : VEvent(),
     rawData(0),
...
bool EventKLDCTracking::ProcessingBegin()
{
 return true;
}

bool EventKLDCTracking::ProcessingNormal( TFile* iFile, int evnum )
{
   ...
   for(int i_entry = 0; i_entry < n_entry; i_entry++){
     ...
     //KLDC Rawdata
#if check
       std::cout << "KLDC Rawdata***********************" << std::endl;
#endif
       ...
       //KLDC Decode
#if check
       std::cout << "KLDC Decode***********************" << std::endl;
#endif

       const int dctdclow = confMan->KLDCTRangeLow();
       const int dctdchigh = confMan->KLDCTRangeHigh();

       DCAna->DecodeKLDCRawHits( rawData );  

       int multi_DC[NumOfLayersKLDC];
       for( int i=0; i<NumOfLayersKLDC; i++ ) multi_DC[i]=0;
       {
          for( int layer=1; layer<=NumOfLayersKLDC; ++layer ){
             const DCHitContainer &cont = DCAna->GetKLDCHC(layer);
             int nh = cont.size();
...
#endif
       event.dt_l1.resize(NumOfWireKLDC);
       event.dt_l2.resize(NumOfWireKLDC);
       event.dt_l3.resize(NumOfWireKLDC);
       event.dt_l4.resize(NumOfWireKLDC);
       event.dt_l5.resize(NumOfWireKLDC);
       event.dt_l6.resize(NumOfWireKLDC);
       event.dt_l7.resize(NumOfWireKLDC);
       event.dt_l8.resize(NumOfWireKLDC);

       event.dl_l1.resize(NumOfWireKLDC);
       event.dl_l2.resize(NumOfWireKLDC);
       event.dl_l3.resize(NumOfWireKLDC);
       event.dl_l4.resize(NumOfWireKLDC);
       event.dl_l5.resize(NumOfWireKLDC);
       event.dl_l6.resize(NumOfWireKLDC);
       event.dl_l7.resize(NumOfWireKLDC);
       event.dl_l8.resize(NumOfWireKLDC);

       event.pos_l1.resize(NumOfWireKLDC);
       event.pos_l2.resize(NumOfWireKLDC);
       event.pos_l3.resize(NumOfWireKLDC);
       event.pos_l4.resize(NumOfWireKLDC);
       event.pos_l5.resize(NumOfWireKLDC);
       event.pos_l6.resize(NumOfWireKLDC);
       event.pos_l7.resize(NumOfWireKLDC);
       event.pos_l8.resize(NumOfWireKLDC);

       event.res_l1.resize(NumOfWireKLDC);
       event.res_l2.resize(NumOfWireKLDC);
       event.res_l3.resize(NumOfWireKLDC);
       event.res_l4.resize(NumOfWireKLDC);
       event.res_l5.resize(NumOfWireKLDC);
       event.res_l6.resize(NumOfWireKLDC);
       event.res_l7.resize(NumOfWireKLDC);
       event.res_l8.resize(NumOfWireKLDC);
       
       {
          ...
          for( int it=0; it<nt; ++it ){
             ...
             for( int ih=0; ih<nh; ++ih ){
                ...
                int layerId=hit->GetLayer()-PlOffsKLDC; 
                double wire=hit->GetWire();
...
void EventKLDCTracking::InitializeEvent( void )
{
   //////Timing counter
   event.nhits_utof = -1;
   event.nhits_ltof = -1;
...
bool EventKLDCTracking::ProcessingEnd()
{
  // gFile->Write();
  // gFile->Close();
...
{  
   ...
   //KLDC Histograms
   for( int i=1; i<=NumOfLayersKLDC; ++i ){
      std::ostringstream title1, title2, title3, title4, title5;
      title1 << "#Hits KLDC#" << std::setw(2) << i;
      title2 << "Hitpat KLDC#" << std::setw(2) << i;
      title3 << "Tdc KLDC#" << std::setw(2) << i;
      title4 << "Drift Time KLDC#" << std::setw(2) << i;
      title5 << "Drift Length KLDC#" << std::setw(2) << i;

      HB1( 100*i+0, title1.str().c_str(), NumOfWireKLDC+1, 0., double(NumOfWireKLDC+1) );
      HB1( 100*i+1, title2.str().c_str(), NumOfWireKLDC+1, 0., double(NumOfWireKLDC+1) );
      HB1( 100*i+2, title3.str().c_str(), 400, 900, 1300 );
      HB1( 100*i+3, title4.str().c_str(), 350, -50., 300. );
      HB1( 100*i+4, title5.str().c_str(),  55, -0.5, 5.0 );
      ...
      HB1( 100*i+11, title10.str().c_str(), NumOfWireKLDC+1, 0., double(NumOfWireKLDC+1) );
      HB1( 100*i+12, title11.str().c_str(), 170, -50, 120 );
      HB1( 100*i+13, title12.str().c_str(),  55, -0.5, 5.0 );
      HB1( 100*i+14, title13.str().c_str(), 300, -300., 300. ); 
      ...
      for (int wire=1; wire<=NumOfWireKLDC; wire++) {
