...
void print_branches(const char* filepath, const int process_entries){
    ...
    for(int i=0; i<6; i++){
        ...
        h_res[i] = new TH1F(Form("h_res%d", i+1), Form("residual of layer %d", i+1), 400, -0.5, 0.5);
    ...
    */
    ...
    TH1F* h_x0 = h_x0y0 -> ProjectionX("h_x0");
    h_x0 -> GetXaxis() -> SetLabelSize(0.05);
    h_x0 -> GetYaxis() -> SetLabelSize(0.05);
    h_x0 -> SetTitle("Track position (X projection)");
    h_x0 -> SetXTitle("x [mm]");
    ...
    TH1F* h_y0 = h_x0y0 -> ProjectionX("h_y0");
    h_y0 -> GetXaxis() -> SetLabelSize(0.05);
    h_y0 -> GetYaxis() -> SetLabelSize(0.05);
    h_y0 -> SetTitle("Track position (Y projection)");
    h_y0 -> SetXTitle("y [mm]");
