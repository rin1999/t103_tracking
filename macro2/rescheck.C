...
void rescheck(const char* filename, int exclude_layer, int max_entries){
...
// Fill histograms
   ...
   for(int ientry=0; ientry<nentries; ientry++){
      ...
      for(int ilayer=0; ilayer<nlayer; ilayer++){
         //if(ilayer==exclude_layer-1) continue;
         for(int ifiber=0; ifiber<nfiber; ifiber++){
...
// Printing
   ...
   string savepath = "image/bft/" + filen + "_trackAnalyzer_ex.pdf";
   ...
   g_res_fiber[exclude_layer-1] -> SetTitle(Form("Residual for each fiber (layer %d);fiber ID;Residual [mm]", exclude_layer));
   g_res_fiber[exclude_layer-1] -> GetXaxis() -> SetRangeUser(0,256);
   g_res_fiber[exclude_layer-1] -> GetYaxis() -> SetRangeUser(-1.,1.);
   g_res_fiber[exclude_layer-1] -> GetXaxis() -> SetLabelSize(0.07);
   g_res_fiber[exclude_layer-1] -> GetYaxis() -> SetLabelSize(0.07);
   g_res_fiber[exclude_layer-1] -> GetXaxis() -> SetTitleOffset(1.05);
   g_res_fiber[exclude_layer-1] -> GetXaxis() -> SetTitleSize(0.07);
   g_res_fiber[exclude_layer-1] -> GetYaxis() -> SetTitleOffset(0.6);
   g_res_fiber[exclude_layer-1] -> GetYaxis() -> SetTitleSize(0.07);
   g_res_fiber[exclude_layer-1] -> SetMarkerStyle(10);
   g_res_fiber[exclude_layer-1] -> Draw("AP");
   /*
   for(int ilayer=0; ilayer<nlayer; ilayer++){
      gStyle -> SetTitleSize(0.07, "t"); 
