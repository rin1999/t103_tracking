...
void track_analyzer_5layers(const char* filename, int max_entries){
...
// Fill histograms
   ...
   for(int ientry=0; ientry<nentries; ientry++){
      ...
      int checksum_layer = 0;
      int layer_not_used = 0;
      if(layer->size()==5){
         for(int i=0; i<5; i++) checksum_layer += layer->at(i);
      }
      switch (checksum_layer)
      {
      case 15:
         layer_not_used = 6;
         break;
      ...
      if(layer_not_used==1){
         int size_mfiber_l4 = mfiber->at(3).size();
         bool firsthit_l4 = true;
         bool firsthit_l1 = true;
         for(int i=0; i<size_mfiber_l4; i++){
            if(99.5<=mfiber->at(3).at(i) && mfiber->at(3).at(i)<=100.5 && (-0.01<u0->at(0)&&u0->at(0)<0.01) && (-0.01<v0->at(0)&&v0->at(0)<0.01)){
               if(firsthit_l4){
                  count_bunbo++;
                  firsthit_l4 = false;
               }
               int size_mfiber_l1 = mfiber->at(0).size();
               for(int j=0; j<size_mfiber_l1; j++){
                  if(97<=mfiber->at(0).at(j)&&mfiber->at(0).at(j)<=103){
                     if(firsthit_l1){
                        count_bunshi++;
                        firsthit_l1 = false;
                     }
...
// Printing
   ...
   */
   ...
   cout << "The eff of fiberID 100 in layer 1 is: " << (double)count_bunshi/count_bunbo << endl;
