...
const double MaxChiSqr = 1000.;
const double MinDeltaChiSqrR = 0.002;

Scat1Track::Scat1Track( double IniP, 
			PreInTrack *In, 
			PreOutTrack *Out )
  : In_(In), Out_(Out),
    iniP_(IniP+DefPini), 
...
Scat1Track::~Scat1Track()
{
  clearHitArray();
}

TrackHit *Scat1Track::GetHit( std::size_t nth ) const
{
...
TrackHit *Scat1Track::GetHitOfLayerNumber( int lnum ) const
{
...
void Scat1Track::fillHitArray( void )
{
  std::size_t nIn = In_->GetNHits();
  std::size_t nOut = Out_->GetNHits();
  ...
  for( std::size_t i=0; i<nOut; ++i ){
    TrackHit *thit = Out_->GetHit( i );
    hitArray_.push_back( thit );
  }
  //std::cout<< nIn << " " << nOut  <<std::endl;  
}

void Scat1Track::clearHitArray( void )
{
...
bool Scat1Track::ReCalc( bool applyRecursively )
{
  static const std::string funcname = "[Scat1Track::ReCalc]";
...
bool Scat1Track::doFit( void )
{
  static const std::string funcname = "[Scat1Track::doFit]";
  ...
  double xTof = Out_->TofPosition().x();
  double yTof = Out_->TofPosition().y();
  double uTof = Out_->TofMomentum().x()/Out_->TofMomentum().mag(); 
  double vTof = Out_->TofMomentum().y()/Out_->TofMomentum().mag(); 
 ...
 ThreeVector gpTof = TrGeomMan::GetInstance().
  ...
  RKScat1CordParameter iniCord( gpTof, gmTof );
  RKScat1CordParameter prevCord;

  RKScat1HitPointContainer preHPntCont;
  ...
  hPntCont_ = RKScat1makeHPContainer(); 
  RKScat1HitPointContainer prevHPntCont;

  int iIter=0, iIterEf=1;

  while( ++iIter < MaxIteraction ){
    if( !RKScat1trace( iniCord, hPntCont_ ) ){
      // Error 
#ifdef WARNOUT 
      //std::cerr << funcname << ": Error in RKScat1trace. " << std::endl;
#endif
      break;
    }
  ...
  if( !RKScat1traceToLast( hPntCont_ ) )
    status_ = false;
...
bool Scat1Track::doFitAgain( RKScat1CordParameter iniCord )
{
  static const std::string funcname = "[Scat1Track::doFitAgain]";
  ...
  double chiSqr, prevChiSqr=InitialChiSqr;
  double lambdaCri=0.001, estTrhisqr=InitialChiSqr;
...
double Scat1Track::calcChiSqr( const RKScat1HitPointContainer &hpCont ) const
{
  ...
  for( std::size_t i=0; i<nh; ++i ){
    ...
    const RKScat1calcHitPoint &calhp = hpCont.HitPointOfLayer( lnum );
...
bool Scat1Track::
guessNextParameters( const RKScat1HitPointContainer &hpCont,
		     RKScat1CordParameter &Cord, double &estDeltaChisqr, 
		     double &lambdaCri,  double dmp ) const
{
  static const std::string funcname = "[Scat1Track::geussNextParameters]";
  ...
  Cord = RKScat1CordParameter( Cord.X(),
			       Cord.Y()+dcb[0],
			       Cord.Z()+dcb[1],
			       Cord.U()+dcb[2],
			       Cord.V()+dcb[3],
...
void Scat1Track::saveCalcPosition( const RKScat1HitPointContainer &hpCont )
{
  static const std::string funcname = "[Scat1Track::saveCalcPosition]";
...
void Scat1Track::PrintCalcHits( const RKScat1HitPointContainer &hpCont,
				std::ostream &ost ) const
{
  std::size_t ndh = hitArray_.size();

  RKScat1HitPointContainer::RKScat1HpCIterator 
  ...
  for( ; itr!=end; ++itr ){
    int lnum = itr->first;
    const RKScat1calcHitPoint &calhp = itr->second;
...
bool Scat1Track::saveTrackParameters( const RKScat1CordParameter &cp )
{
  CPval_ = cp;
  ...
  const RKScat1calcHitPoint &hpTof  = hPntCont_.HitPointOfLayer( TOFid );
  const RKScat1calcHitPoint &hpTgt  = hPntCont_.begin()->second;
  const RKScat1calcHitPoint &hpLast = hPntCont_.rbegin()->second;
...
GetTrajectoryLocalPosition( int layer, double & x, double & y ) const
{
  static const std::string funcname = 
    "[Scat1Track:GetTrajectoryLocalPosition]";

  try {
    const RKScat1calcHitPoint &HP=hPntCont_.HitPointOfLayer( layer );
