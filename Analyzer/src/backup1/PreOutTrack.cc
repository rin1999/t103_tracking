...
PreOutTrack::PreOutTrack( double IniP, 
			  TrLocalTrack *Out1, 
			  TrLocalTrack *Out2 )
...
PreOutTrack::~PreOutTrack()
{
  clearHitArray();
}

TrackHit *PreOutTrack::GetHit( std::size_t nth ) const
{
...
TrackHit *PreOutTrack::GetHitOfLayerNumber( int lnum ) const
{
...
void PreOutTrack::fillHitArray( void )
{
  std::size_t nOut1 = Out1_->GetNHit();
  std::size_t nOut2 = Out2_->GetNHit();
 
  clearHitArray();
  hitArray_.reserve( nOut1+nOut2 );

  for( std::size_t i=0; i<nOut1; ++i ){
    TrLTrackHit *hit = Out1_->GetHit( i );
    TrackHit *thit = new TrackHit( hit );
    hitArray_.push_back( thit );
  }

  for( std::size_t i=0; i<nOut2; ++i ){
    TrLTrackHit *hit = Out2_->GetHit( i );
    TrackHit *thit = new TrackHit( hit );
    hitArray_.push_back( thit );
  }

  //std::cout<< nOut1 << " " << nOut2 <<std::endl;  
}

void PreOutTrack::clearHitArray( void )
{
...
bool PreOutTrack::ReCalc( bool applyRecursively )
{
  static const std::string funcname = "[PreOutTrack::ReCalc]";

  if( applyRecursively ){
    Out1_->ReCalc(applyRecursively);
    Out2_->ReCalc(applyRecursively);
...
bool PreOutTrack::doFit( void )
{
  static const std::string funcname = "[PreOutTrack::doFit]";
  ...
  double xTof = Out2_->GetX( zTof );
  double yTof = Out2_->GetY( zTof );
  double uTof = Out2_->GetU0(); 
  double vTof = Out2_->GetV0(); 
 ...
 ThreeVector gpTof = TrGeomMan::GetInstance().
  ...
  RKPreOutCordParameter iniCord( gpTof, gmTof );
  RKPreOutCordParameter prevCord;

  RKPreOutHitPointContainer preHPntCont;
  ...
  hPntCont_ = RKPreOutmakeHPContainer(); 
  RKPreOutHitPointContainer prevHPntCont;

  int iIter=0, iIterEf=1;

  while( ++iIter < MaxIteraction ){
    if( !RKPreOuttrace( iniCord, hPntCont_ ) ){
      // Error 
#ifdef WARNOUT 
      //std::cerr << funcname << ": Error in RKPreOuttrace. " << std::endl;
#endif
      break;
    }
  ...
  if( !RKPreOuttraceToLast( hPntCont_ ) )
    status_ = false;
...
bool PreOutTrack::doFitAgain( RKPreOutCordParameter iniCord )
{
  static const std::string funcname = "[PreOutTrack::doFitAgain]";
...
double PreOutTrack::calcChiSqr( const RKPreOutHitPointContainer &hpCont ) const
{
  ...
  for( std::size_t i=0; i<nh; ++i ){
    ...
    const RKPreOutcalcHitPoint &calhp = hpCont.HitPointOfLayer( lnum );
...
bool PreOutTrack::
guessNextParameters( const RKPreOutHitPointContainer &hpCont,
		     RKPreOutCordParameter &Cord, double &estDeltaChisqr, 
		     double &lambdaCri,  double dmp ) const
{
  static const std::string funcname = "[PreOutTrack::geussNextParameters]";
  ...
  Cord = RKPreOutCordParameter( Cord.X(),
				Cord.Y()+dcb[0],
				Cord.Z()+dcb[1],
				Cord.U()+dcb[2],
				Cord.V()+dcb[3],
...
void PreOutTrack::saveCalcPosition( const RKPreOutHitPointContainer &hpCont )
{
  static const std::string funcname = "[PreOutTrack::saveCalcPosition]";
...
void PreOutTrack::PrintCalcHits( const RKPreOutHitPointContainer &hpCont,
				 std::ostream &ost ) const
{
  std::size_t ndh = hitArray_.size();

  RKPreOutHitPointContainer::RKPreOutHpCIterator 
  ...
  for( ; itr!=end; ++itr ){
    int lnum = itr->first;
    const RKPreOutcalcHitPoint &calhp = itr->second;
...
bool PreOutTrack::saveTrackParameters( const RKPreOutCordParameter &cp )
{
  CPval_ = cp;
  const TrGeomMan &geomMan = TrGeomMan::GetInstance();
  int TOFid = geomMan.GetDetectorId("TOF");
  int TGTid = hPntCont_.begin()->first;

  const RKPreOutcalcHitPoint &hpTof  = hPntCont_.HitPointOfLayer( TOFid );
  const RKPreOutcalcHitPoint &hpTgt  = hPntCont_.begin()->second;
  const RKPreOutcalcHitPoint &hpLast = hPntCont_.rbegin()->second;
...
GetTrajectoryLocalPosition( int layer, double & x, double & y ) const
{
  static const std::string funcname = 
    "[PreOutTrack:GetTrajectoryLocalPosition]";

  try {
    const RKPreOutcalcHitPoint &HP=hPntCont_.HitPointOfLayer( layer );
