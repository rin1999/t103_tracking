...
BeamTrack::BeamTrack( TrLocalTrack *In )
  : In_(In), iniP_(DefPini), 
...
TrackHit *BeamTrack::GetHit( std::size_t nth ) const
{
...
TrackHit *BeamTrack::GetHitOfLayerNumber( int lnum ) const
{
...
void BeamTrack::fillHitArray( void )
{
  std::size_t nIn = In_->GetNHit();
  clearHitArray();
  hitArray_.reserve( nIn );

  for( std::size_t i=0; i<nIn; ++i ){
    TrLTrackHit *hit = In_->GetHit( i );
    TrackHit *thit = new TrackHit( hit );
    hitArray_.push_back( thit );
  }
}

void BeamTrack::clearHitArray( void )
{
...
bool BeamTrack::ReCalc( bool applyRecursively )
{
  static const std::string funcname = "[BeamTrack::ReCalc]";
...
bool BeamTrack::doFit( void )
{
  static const std::string funcname = "[BeamTrack::doFit]";

  //  clearHitArray();
  //  fillHitArray();

  int IdVTg = TrGeomMan::GetInstance().GetDetectorId("VTarget");
  double zVTg = TrGeomMan::GetInstance().GetLocalZ( IdVTg ); 
  double xVTg = In_->GetX( zVTg );
  double yVTg = In_->GetY( zVTg );
  double uVTg = In_->GetU0(); 
  double vVTg = In_->GetV0(); 

  ThreeVector gpVTg = TrGeomMan::GetInstance().
    Local2GlobalPos( IdVTg, ThreeVector( xVTg, yVTg, 0.0 ) );
  double pz = iniP_/sqrt(1.+uVTg*uVTg+vVTg*vVTg);
  ThreeVector gmVTg = TrGeomMan::GetInstance().
    Local2GlobalDir( IdVTg, ThreeVector( pz*uVTg, pz*vVTg, pz ) );
  ...
  RKBeamCordParameter iniCord( gpVTg, gmVTg );
  RKBeamCordParameter prevCord;

  RKBeamHitPointContainer preHPntCont;
  ...
  hPntCont_ = RKBeammakeHPContainer(); 
  RKBeamHitPointContainer prevHPntCont;

  int iIter=0, iIterEf=1;

  while( ++iIter < MaxIteraction ){
    if( !RKBeamtrace( iniCord, hPntCont_ ) ){
      // Error 
#ifdef WARNOUT 
      //std::cerr << funcname << ": Error in RKBeamtrace. " << std::endl;
#endif
      break;
    }
  ...
  if( !RKBeamtraceToLast( hPntCont_ ) )
    status_ = false;
...
bool BeamTrack::doFitAgain( RKBeamCordParameter iniCord )
{
  static const std::string funcname = "[BeamTrack::doFitAgain]";
...
double BeamTrack::calcChiSqr( const RKBeamHitPointContainer &hpCont ) const
{
  ...
  for( std::size_t i=0; i<nh; ++i ){
    ...
    const RKBeamcalcHitPoint &calhp = hpCont.HitPointOfLayer( lnum );
...
bool BeamTrack::
guessNextParameters( const RKBeamHitPointContainer &hpCont,
		     RKBeamCordParameter &Cord, double &estDeltaChisqr, 
		     double &lambdaCri,  double dmp ) const
{
  static const std::string funcname = "[BeamTrack::geussNextParameters]";
  ...
  Cord = RKBeamCordParameter( Cord.X(),
			      Cord.Y()+dcb[0],
			      Cord.Z()+dcb[1],
			      Cord.U()+dcb[2],
			      Cord.V()+dcb[3],
...
void BeamTrack::saveCalcPosition( const RKBeamHitPointContainer &hpCont )
{
  static const std::string funcname = "[BeamTrack::saveCalcPosition]";
...
void BeamTrack::PrintCalcHits( const RKBeamHitPointContainer &hpCont,
			      std::ostream &ost ) const
{
  std::size_t ndh = hitArray_.size();

  RKBeamHitPointContainer::RKBeamHpCIterator 
  ...
  for( ; itr!=end; ++itr ){
    int lnum = itr->first;
    const RKBeamcalcHitPoint &calhp = itr->second;
...
bool BeamTrack::saveTrackParameters( const RKBeamCordParameter &cp )
{
  CPval_ = cp;
  const TrGeomMan &geomMan = TrGeomMan::GetInstance();

  int VD1id = geomMan.GetDetectorId("SpecVp1");
  int TGTid = geomMan.GetDetectorId("VTarget");

  const RKBeamcalcHitPoint &hpTgt  = hPntCont_.HitPointOfLayer( TGTid );
  const RKBeamcalcHitPoint &hpFirts= hPntCont_.HitPointOfLayer( TGTid );
  const RKBeamcalcHitPoint &hpLast = hPntCont_.HitPointOfLayer( VD1id );
  ...
  pLenTtoT_ = fabs( hpTgt.PathLength()-hpLast.PathLength() );
  pLenTot_  = fabs( hpTgt.PathLength()-hpLast.PathLength() );

  // int VTgid = geomMan.GetDetectorId("SpecVp1");
  // int TGTid = hPntCont_.begin()->first;

  // const RKBeamcalcHitPoint &hpVTg  = hPntCont_.HitPointOfLayer( VTgid );
  // const RKBeamcalcHitPoint &hpTgt  = hPntCont_.begin()->second;
  // const RKBeamcalcHitPoint &hpLast = hPntCont_.rbegin()->second;
  ...
  // pLenTtoT_ = fabs( hpTgt.PathLength()-hpVD1.PathLength() );
  // pLenTot_  = fabs( hpTgt.PathLength()-hpLast.PathLength() );
...
GetTrajectoryLocalPosition( int layer, double & x, double & y ) const
{
  static const std::string funcname = 
    "[BeamTrack:GetTrajectoryLocalPosition]";

  try {
    const RKBeamcalcHitPoint &HP=hPntCont_.HitPointOfLayer( layer );
