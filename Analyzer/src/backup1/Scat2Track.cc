...
Scat2Track::Scat2Track( double IniP, 
			PreInTrack *In, 
			PreOut2Track *Out,
			int type )
  : In_(In), Out_(Out),
    iniP_(IniP+DefPini), type_(type),
...
Scat2Track::~Scat2Track()
{
  clearHitArray();
}

TrackHit *Scat2Track::GetHit( std::size_t nth ) const
{
...
TrackHit *Scat2Track::GetHitOfLayerNumber( int lnum ) const
{
...
void Scat2Track::fillHitArray( void )
{
...
void Scat2Track::clearHitArray( void )
{
...
bool Scat2Track::ReCalc( bool applyRecursively )
{
  static const std::string funcname = "[Scat2Track::ReCalc]";
...
bool Scat2Track::doFit( void )
{
  static const std::string funcname = "[Scat2Track::doFit]";
  ...
  if( type_==10 ){
    ...
    xTof = Out_->TofPosition().x();
    yTof = Out_->TofPosition().y();
    uTof = Out_->TofMomentum().x()/Out_->TofMomentum().mag(); 
    vTof = Out_->TofMomentum().y()/Out_->TofMomentum().mag(); 
  ...
  RKScat2CordParameter iniCord( gpTof, gmTof );
  RKScat2CordParameter prevCord;

  RKScat2HitPointContainer preHPntCont;
  ...
  hPntCont_ = RKScat2makeHPContainer( type_ ); 
  RKScat2HitPointContainer prevHPntCont;

  int iIter=0, iIterEf=1;

  while( ++iIter < MaxIteraction ){
    if( !RKScat2trace( iniCord, hPntCont_ ) ){
      // Error 
#ifdef WARNOUT 
      //std::cerr << funcname << ": Error in RKScat2trace. " << std::endl;
#endif
      break;
    }
  ...
  if( !RKScat2traceToLast( hPntCont_, type_ ) )
    status_ = false;
...
bool Scat2Track::doFitAgain( RKScat2CordParameter iniCord )
{
  static const std::string funcname = "[Scat2Track::doFitAgain]";
...
double Scat2Track::calcChiSqr( const RKScat2HitPointContainer &hpCont ) const
{
  ...
  for( std::size_t i=0; i<nh; ++i ){
    ...
    const RKScat2calcHitPoint &calhp = hpCont.HitPointOfLayer( lnum );
...
bool Scat2Track::
guessNextParameters( const RKScat2HitPointContainer &hpCont,
		     RKScat2CordParameter &Cord, double &estDeltaChisqr, 
		     double &lambdaCri,  double dmp ) const
{
  static const std::string funcname = "[Scat2Track::geussNextParameters]";
  ...
  Cord = RKScat2CordParameter( Cord.X(),
			       Cord.Y()+dcb[0],
			       Cord.Z()+dcb[1],
			       Cord.U()+dcb[2],
			       Cord.V()+dcb[3],
...
void Scat2Track::saveCalcPosition( const RKScat2HitPointContainer &hpCont )
{
  static const std::string funcname = "[Scat2Track::saveCalcPosition]";
...
void Scat2Track::PrintCalcHits( const RKScat2HitPointContainer &hpCont,
				std::ostream &ost ) const
{
  std::size_t ndh = hitArray_.size();

  RKScat2HitPointContainer::RKScat2HpCIterator 
  ...
  for( ; itr!=end; ++itr ){
    int lnum = itr->first;
    const RKScat2calcHitPoint &calhp = itr->second;
...
bool Scat2Track::saveTrackParameters( const RKScat2CordParameter &cp )
{
  CPval_ = cp;
  ...
  const RKScat2calcHitPoint &hpTof  = hPntCont_.HitPointOfLayer( TOFid );
  const RKScat2calcHitPoint &hpTgt  = hPntCont_.begin()->second;
  const RKScat2calcHitPoint &hpLast = hPntCont_.rbegin()->second;
...
GetTrajectoryLocalPosition( int layer, double & x, double & y ) const
{
  static const std::string funcname = 
    "[Scat2Track:GetTrajectoryLocalPosition]";

  try {
    const RKScat2calcHitPoint &HP=hPntCont_.HitPointOfLayer( layer );
