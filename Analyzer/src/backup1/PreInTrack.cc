...
PreInTrack::PreInTrack( double IniP, 
			TrLocalTrack *In1, 
			TrLocalTrack *In2 )
  : In1_(In1), In2_(In2),
    iniP_(IniP+DefPini), 
...
PreInTrack::~PreInTrack()
{
  clearHitArray();
}

TrackHit *PreInTrack::GetHit( std::size_t nth ) const
{
...
TrackHit *PreInTrack::GetHitOfLayerNumber( int lnum ) const
{
...
void PreInTrack::fillHitArray( void )
{
...
void PreInTrack::clearHitArray( void )
{
...
bool PreInTrack::ReCalc( bool applyRecursively )
{
  static const std::string funcname = "[PreInTrack::ReCalc]";
...
bool PreInTrack::doFit( void )
{
  static const std::string funcname = "[PreInTrack::doFit]";
 ...
 ThreeVector gpTof = TrGeomMan::GetInstance().
  ...
  RKPreInCordParameter iniCord( gpTof, gmTof );
  RKPreInCordParameter prevCord;

  RKPreInHitPointContainer preHPntCont;
  ...
  hPntCont_ = RKPreInmakeHPContainer(); 
  RKPreInHitPointContainer prevHPntCont;

  int iIter=0, iIterEf=1;

  while( ++iIter < MaxIteraction ){
    if( !RKPreIntrace( iniCord, hPntCont_ ) ){
      // Error 
#ifdef WARNOUT 
      //std::cerr << funcname << ": Error in RKPreIntrace. " << std::endl;
#endif
      break;
    }
  ...
  if( !RKPreIntraceToLast( hPntCont_ ) )
    status_ = false;
...
bool PreInTrack::doFitAgain( RKPreInCordParameter iniCord )
{
  static const std::string funcname = "[PreInTrack::doFitAgain]";
...
double PreInTrack::calcChiSqr( const RKPreInHitPointContainer &hpCont ) const
{
  ...
  for( std::size_t i=0; i<nh; ++i ){
    ...
    const RKPreIncalcHitPoint &calhp = hpCont.HitPointOfLayer( lnum );
...
bool PreInTrack::
guessNextParameters( const RKPreInHitPointContainer &hpCont,
		     RKPreInCordParameter &Cord, double &estDeltaChisqr, 
		     double &lambdaCri,  double dmp ) const
{
  static const std::string funcname = "[PreInTrack::geussNextParameters]";
  ...
  Cord = RKPreInCordParameter( Cord.X(),
			       Cord.Y()+dcb[0],
			       Cord.Z()+dcb[1],
			       Cord.U()+dcb[2],
			       Cord.V()+dcb[3],
...
void PreInTrack::saveCalcPosition( const RKPreInHitPointContainer &hpCont )
{
  static const std::string funcname = "[PreInTrack::saveCalcPosition]";
...
void PreInTrack::PrintCalcHits( const RKPreInHitPointContainer &hpCont,
				std::ostream &ost ) const
{
  std::size_t ndh = hitArray_.size();

  RKPreInHitPointContainer::RKPreInHpCIterator 
  ...
  for( ; itr!=end; ++itr ){
    int lnum = itr->first;
    const RKPreIncalcHitPoint &calhp = itr->second;
...
bool PreInTrack::saveTrackParameters( const RKPreInCordParameter &cp )
{
  CPval_ = cp;
  ...
  const RKPreIncalcHitPoint &hpTof  = hPntCont_.HitPointOfLayer( TOFid );
  const RKPreIncalcHitPoint &hpTgt  = hPntCont_.begin()->second;
  const RKPreIncalcHitPoint &hpLast = hPntCont_.rbegin()->second;
...
GetTrajectoryLocalPosition( int layer, double & x, double & y ) const
{
  static const std::string funcname = 
    "[PreInTrack:GetTrajectoryLocalPosition]";

  try {
    const RKPreIncalcHitPoint &HP=hPntCont_.HitPointOfLayer( layer );
