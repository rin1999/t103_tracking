			   ...
			   unsigned int L1_Tdc, unsigned int L1_Tdc1 )
{
  ...
  if(p){
    ...
    //std::cout<< "Layer=" << Layer << " :Ch=" << Seg << std::endl;
...
bool RawData::DecodeRawHits( Decoder& gDec )
{
  ...
  /////////////////////////////////////////////////////////
  //Fiber decoding
  int FiberAdcHigh[NumOfFiber0], FiberAdcLow[NumOfFiber0];
  std::vector<std::vector<int> > FiberTdcLeading;
  std::vector<std::vector<int> > FiberTdcTrailing;

  FiberTdcLeading.resize(NumOfFiber0);
  FiberTdcTrailing.resize(NumOfFiber0);
  ...
  for( int ich=0; ich<NumOfFiber0; ich++ ){
