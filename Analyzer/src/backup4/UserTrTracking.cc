...
bool EventTrTracking::ProcessingNormal( Decoder& gDec )
{
  ...
  bool tdcflag[NumOfLayersSFT][MaxHits];
...

  {
	...
	id=MaxFiber*(layerId-1)+(fiberId-1-121);

	//std::cout << " ID = " << MaxFiber*(layerId-1)+(fiberId-1-121)<< std::endl;
 ...
 bool trackflag=false;
 {
  ...
  // std::cout<< "NTrack = " << nt << std::endl;
    ...
    for( int it=0; it<nt; ++it ){
      ...
      double xtgt=tp->GetX(  699.05 ), ytgt=tp->GetY(  699.05 );
...
{  
  ...
  tree->Branch("sftnhits",  event.sftnhits, "sftnhits[64]/I");
