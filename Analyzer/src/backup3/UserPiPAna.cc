...
// TOF Cut Parameters
const double MinDeltaETof = 1.0; //for Proton
const double MaxDeltaETof = 6.0; //for Proton
const double MinTimeTof =  20.0;
const double MaxTimeTof =  36.0;

// LC Cut Parameters
const double MinDeltaELc = 0.00; //for Kaon
const double MaxDeltaELc = 5.00; //for Kaon
const double MinTimeLc =   0.0;
const double MaxTimeLc = 100.0;
...
const double MinMassSquare =  0.70;
const double MaxMassSquare =  1.00;
...
bool EventPiPAna::ProcessingBegin()
...
bool EventPiPAna::ProcessingNormal()
{
  rawData = new RawData;
  rawData->DecodeHits();
  ...
  HF1( 301, double(ncTof) );
  {
	...
	event.deTof[i]=de;
      ...
      //------------------------Cut
      if( MinTimeTof  <t  && t< MaxTimeTof && (cl->MeanSeg()+1)>8 ){
	++ncOk;
...
// 	std::cout<<"m2= "<< m2 <<std::endl;
  ...
  for( int ik=0; ik<nK; ++ik ){
    ...
    for( int ipi=0; ipi<nPi; ++ipi ){
      ...
      LorentzVector LvK( pk, sqrt(ProtonMass*ProtonMass+pk.mag2()) );
      ...
      double BE=LvRc.mag()-( LvCore.mag()+ProtonMass );
	...
	= 0.5*sqrt(( TotalEnergyCM*TotalEnergyCM
		     -( ProtonMass+PionMass )*( ProtonMass+PionMass ))
		   *( TotalEnergyCM*TotalEnergyCM
		      -( ProtonMass-PionMass )*( ProtonMass-PionMass )))/TotalEnergyCM;
      
      double costLab = cost;
      double cottLab = costLab/sqrt(1.-costLab*costLab);
      double bt=beta.mag(), gamma=1./sqrt(1.-bt*bt);
      double gbep=gamma*bt*sqrt(TotalMomCM*TotalMomCM+ProtonMass*ProtonMass)/TotalMomCM;
...
void EventPiPAna::InitializeEvent( void )
{
...
bool EventPiPAna::ProcessingEnd()
{
  // gFile->Write();
  // gFile->Close();
  return true;
