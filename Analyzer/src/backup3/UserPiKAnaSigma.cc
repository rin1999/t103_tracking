...
EventPiKAnaSigma::EventPiKAnaSigma()
  : VEvent(),
    rawData(0), 
...
struct Event{
  ...
  double BE[MaxHits];
  double thetaPi_CM[MaxHits];
  double thetaK_CM[MaxHits];
  double thetaS_CM[MaxHits];
...
const double MaxDeltaETof = 4.0; //for Kaon
const double MinTimeTof =  15.0;
const double MaxTimeTof =  35.0;

// LC Cut Parameters
const double MinDeltaELc = 0.03; //for Kaon
...
const double MaxModTimeTof = 24.0;
const double MinModTimeLc  = 25.5;
const double MaxModTimeLc  = 31.0;

// TOF-SdcOut
const double MinXDifTof  =  -70.;
const double MaxXDifTof  =   70.;
const double MinYDifTof  =  -200.;
const double MaxYDifTof  =   200.;

// LC-SdcOut
const double MinXDifLc  = -150.;
const double MaxXDifLc  =  150.;
const double MinYDifLc  = -800.;
const double MaxYDifLc  =  800.;

//Multi cut of DCs
const double MaxMultiHitBcIn   = 2.5;
...
const double MaxMassSquare =  0.50;
// const double MinMassSquare =  0.20;
// const double MaxMassSquare =  0.45;
...
bool EventPiKAnaSigma::ProcessingBegin()
...
bool EventPiKAnaSigma::ProcessingNormal()
{
  rawData = new RawData;
  rawData->DecodeHits();
...
// 	std::cout<<"m2= "<< m2 <<std::endl;
  ...
  for( int ik=0; ik<nK; ++ik ){
    ...
    for( int ipi=0; ipi<nPi; ++ipi ){
      ...
      std::cout<<"******* Missing Mass= "<< MisMass <<std::endl;

      double us=pk.x()/pk.z(), vs=pk.y()/pk.z();
      double ub=ppi.x()/ppi.z(), vb=ppi.y()/ppi.z();
      double cost=ppi*pk/(ppi.mag()*pk.mag());

      ThreeVector beta( (LvPi+LvC).vect()/(LvPi+LvC).e() );
      LvPi.boost ( (-1.)*beta );
      LvK.boost ( (-1.)*beta );
      LvRc.boost ( (-1.)*beta );

      double cost_pi = LvPi.vect().theta();
      double cost_k  = LvK.vect().theta();
      double cost_s  = LvRc.vect().theta();
	...
	event.thetaPi_CM[npik]=acos(cost_pi)*Rad2Deg;
	event.thetaK_CM[npik] =acos(cost_k)*Rad2Deg;
	event.thetaS_CM[npik] =acos(cost_s)*Rad2Deg;
...
void EventPiKAnaSigma::InitializeEvent( void )
{
  ...
  for( int it=0; it<MaxHits; it++){
    ...
    event.thetaPi_CM[it] = -9999.0; 
    event.thetaK_CM[it]  = -9999.0; 
    event.thetaS_CM[it]  = -9999.0; 
...
bool EventPiKAnaSigma::ProcessingEnd()
{
  // gFile->Write();
  // gFile->Close();
  return true;
...
{  
  ...
  tree->Branch("thetaPi_CM", event.thetaPi_CM, "thetaPi_CM[nPiK]/D");
  tree->Branch("thetaK_CM",  event.thetaK_CM,  "thetaK_CM[nPiK]/D");
  tree->Branch("thetaS_CM",  event.thetaS_CM,  "thetaS_CM[nPiK]/D");
