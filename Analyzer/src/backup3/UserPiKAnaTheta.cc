...
EventPiKAnaTheta::EventPiKAnaTheta()
  : VEvent(),
    rawData(0), 
...
bool EventPiKAnaTheta::ProcessingBegin()
...
bool EventPiKAnaTheta::ProcessingNormal()
{
  rawData = new RawData;
  rawData->DecodeHits();
  ...
  int trig=0;
  {
    ...
    for( int i=0; i<nh; ++i ){
      ...
      //(pi, K) w/o MT
      //if( seg==13 && T>0 ){ trig_type = seg; event.trigtype = seg;}
  ...
  //if (trig_type != WOMT_TRIG )  return true;
  HF1( 1, 1. );
...
//   std::cout << "Flag==" << matrix_flag <<std::endl;
...

  {
	...
	event.pK18[i]=pk18;
	event.xtb[i] = track->Xtgt();
	event.ytb[i] = track->Ytgt();
	event.utb[i] = track->Utgt();
	event.vtb[i] = track->Vtgt();
      }
...
void EventPiKAnaTheta::InitializeEvent( void )
{
...
bool EventPiKAnaTheta::ProcessingEnd()
{
  // gFile->Write();
  // gFile->Close();
  return true;
