...
EventPiKAnaSigmaMinus::EventPiKAnaSigmaMinus()
  : VEvent(),
    rawData(0), 
...
//Multi cut of DCs
//For study
// const double MaxMultiHitBcIn   = 2.0;
// const double MaxMultiHitBcOut  = 5.;
// const double MaxMultiHitSdcIn  = 5.;
...
bool EventPiKAnaSigmaMinus::ProcessingBegin()
...
bool EventPiKAnaSigmaMinus::ProcessingNormal()
{
  rawData = new RawData;
  rawData->DecodeHits();
...
// 	std::cout<<"m2= "<< m2 <<std::endl;
  ...
  for( int ik=0; ik<nK; ++ik ){
    ...
    for( int ipi=0; ipi<nPi; ++ipi ){
      ...
      double pu0 =  0.0;
      double pu1 =  0.0020731;
      double pu2 =  0.00665803;
      double pu3 = -0.0525663;
      double pu4 =  0.451844;

      double pv0 = -0.0000202;
      double pv1 = -0.000702903;
      double pv2 = -0.00402638;
      double pv3 =  0.0718652;
      double pv4 =  4.72381;
	...
	= 0.5*sqrt(( TotalEnergyCM*TotalEnergyCM
		     -( KaonMass+SigmaMMass )*( KaonMass+SigmaMMass ))
		   *( TotalEnergyCM*TotalEnergyCM
		      -( KaonMass-SigmaMMass )*( KaonMass-SigmaMMass )))/TotalEnergyCM;
...
void EventPiKAnaSigmaMinus::InitializeEvent( void )
{
...
bool EventPiKAnaSigmaMinus::ProcessingEnd()
{
  // gFile->Write();
  // gFile->Close();
  return true;
...
{
  return new EventPiKAnaSigmaMinus;
}
